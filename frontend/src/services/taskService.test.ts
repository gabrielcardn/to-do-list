...
import {
  getTasks,
  createTask,
  // Importe outras funções e tipos de taskService conforme for testando
  type Task,
...
describe('taskService', () => {
  ...
  beforeEach(() => {
    ...
    localStorage.setItem('accessToken', mockToken); // Simula um usuário logado
  });
  ...
  describe('getTasks', () => {
    const mockPaginatedResponse: PaginatedTasksResponse = {
      data: [{ id: 'task1', title: 'Tarefa 1', description: 'Desc 1', status: 'PENDING', userId: 'user1' }],
      total: 1,
      page: 1,
      limit: 10,
    };
    ...
    it('should fetch tasks successfully with a token', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        `${API_BASE_URL}/tasks?page=1&limit=10`,
        {
          method: 'GET',
          headers: {
    ...
    it('should throw an error if no access token is found', async () => {
      localStorage.removeItem('accessToken'); // Simula usuário não logado
      await expect(getTasks()).rejects.toThrow('Nenhum token de acesso encontrado. Faça login novamente.');
    ...
    it('should handle 401 error by removing token and throwing specific error', async () => {
     ...
     expect(initialToken).toBe(mockToken); // Confirma que o token existe antes da chamada

     await expect(getTasks()).rejects.toThrow('Sessão expirada ou token inválido. Faça login novamente.');
     
     const tokenAfterError = localStorage.getItem('accessToken');
     expect(tokenAfterError).toBeNull(); // Confirma que o token foi removido
    });
  });

  // Testes para createTask()
  describe('createTask', () => {
    const mockTaskPayload: CreateTaskPayload = { title: 'Nova Tarefa', description: 'Detalhes da nova tarefa' };
    const mockCreatedTask: Task = { 
      id: 'task-new-uuid', 
      title: 'Nova Tarefa', 
      description: 'Detalhes da nova tarefa', 
      status: 'PENDING', 
      userId: 'user-uuid-123' // O backend associaria o userId
    };
    ...
    it('should throw an error if no access token is found', async () => {
     ...
     await expect(createTask(mockTaskPayload)).rejects.toThrow('Nenhum token de acesso encontrado. Faça login novamente.');
     expect(fetch).not.toHaveBeenCalled();
    });
